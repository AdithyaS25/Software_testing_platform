...
export const Button = ({
  ...
  const variants: Record<BtnVariant, React.CSSProperties> = {
    primary: {
      ...
      color: "#fff",
      boxShadow: "0 4px 16px rgba(120,87,255,0.3), inset 0 1px 0 rgba(255,255,255,0.15)",
    },
    secondary: {
    ...
    <button
      ...
      onMouseEnter={e => {
        ...
        if (variant === "primary") {
          ...
          e.currentTarget.style.boxShadow = "0 8px 24px rgba(120,87,255,0.4), inset 0 1px 0 rgba(255,255,255,0.15)";
...
// ─── Badge ────────────────────────────────────────────────────────────────────
type BadgeColor = "blue" | "green" | "yellow" | "red" | "purple" | "gray" | "orange" | "cyan";

interface BadgeProps { color?: BadgeColor; children: React.ReactNode; dot?: boolean; }

const badgeColors: Record<BadgeColor, { bg: string; text: string; border: string; glow?: string }> = {
  blue:   { bg: "rgba(120,87,255,0.12)",  text: "#a082ff",  border: "rgba(120,87,255,0.25)", glow: "rgba(120,87,255,0.2)" },
  green:  { bg: "rgba(34,217,160,0.1)",   text: "var(--success)",  border: "rgba(34,217,160,0.25)" },
  yellow: { bg: "rgba(255,197,61,0.1)",   text: "var(--warning)",  border: "rgba(255,197,61,0.25)" },
  red:    { bg: "rgba(255,79,109,0.1)",   text: "var(--danger)",   border: "rgba(255,79,109,0.25)" },
  purple: { bg: "rgba(192,132,252,0.1)",  text: "var(--purple)",   border: "rgba(192,132,252,0.25)" },
  gray:   { bg: "rgba(255,255,255,0.05)", text: "var(--text-secondary)", border: "rgba(255,255,255,0.1)" },
  orange: { bg: "rgba(251,146,60,0.1)",   text: "#fb923c",  border: "rgba(251,146,60,0.25)" },
  cyan:   { bg: "rgba(61,217,255,0.1)",   text: "var(--info)",     border: "rgba(61,217,255,0.25)" },
};
...
export const StatusBadge = ({ value }: { value: string }) => {
  const map: Record<string, BadgeColor> = {
    ...
    PASSED: "green", FAILED: "red", COMPLETED: "cyan", IN_PROGRESS_RUN: "orange",
  };
      ...
      <div className="modal" style={{ maxWidth: maxW }}>
        <div className="modal-header">
          ...
          <button
            ...
            onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,79,109,0.1)"; e.currentTarget.style.color = "var(--danger)"; }}
...
export const FormField = ({ label, error, children, required }: { label: string; error?: string; children: React.ReactNode; required?: boolean }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    {children}
    {error && (
      <p style={{ fontSize: "0.75rem", color: "var(--danger)", marginTop: 5, display: "flex", alignItems: "center", gap: 4 }}>
        <span>⚠</span> {error}
      </p>
    )}
  </div>
...
export const EmptyState = ({ icon, title, desc, action }: { icon: string; title: string; desc?: string; action?: React.ReactNode }) => (
  <div style={{
    ...
    <div style={{
      fontSize: "3rem", marginBottom: 16, opacity: 0.4,
    ...
    {desc && <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: 20 }}>{desc}</p>}
    {action}
  </div>
);
...
export const ConfirmDialog = ({ open, onClose, onConfirm, title, message, confirmLabel = "Confirm", variant = "danger" }:
  ...
  <Modal open={open} onClose={onClose} title={title} size="sm">
    <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", marginBottom: 24, lineHeight: 1.7 }}>{message}</p>
  ...
  <div style={{
    ...
    {tabs.map(tab => (
      <button key={tab.id} onClick={() => onChange(tab.id)} style={{
        ...
        border: "1px solid",
        borderColor: active === tab.id ? "rgba(120,87,255,0.25)" : "transparent",
        borderBottom: active === tab.id ? "1px solid var(--accent)" : "1px solid transparent",
        ...
        {tab.count !== undefined && (
          <span style={{
            padding: "1px 7px", borderRadius: "100px", fontSize: "0.68rem",
            background: active === tab.id ? "rgba(120,87,255,0.2)" : "rgba(255,255,255,0.06)",
...
export const SearchInput = ({ value, onChange, placeholder = "Search..." }: { value: string; onChange: (v: string) => void; placeholder?: string }) => (
  <div style={{ position: "relative", display: "inline-flex", alignItems: "center" }}>
    <span style={{ position: "absolute", left: 12, color: "var(--text-muted)", fontSize: "0.9rem", pointerEvents: "none" }}>⌕</span>
    <input
      value={value}
...
export const StatCard = ({ label, value, icon, color = "blue", sub }: { label: string; value: string | number; icon: string; color?: BadgeColor; sub?: string }) => {
  ...
  return (
    ...
    >
      ...
      {sub && <div style={{ fontSize: "0.75rem", color: "var(--text-muted)", marginTop: 7 }}>{sub}</div>}
