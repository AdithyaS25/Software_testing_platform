...
export const TestSuitesPage = () => {
  ...
  const loadTestCases = async () => {
    ...
    try {
      const r = await apiClient.get(`/api/projects/${projectId}/test-cases?limit=200`);
  ...
  const handleClone   = async (id: string) => {
    try { await apiClient.post(`/api/projects/${projectId}/test-suites/${id}/clone`); toast.success("Cloned"); load(); }
    catch { toast.error("Failed to clone"); }
  };

  const handleArchive = async (id: string, archived: boolean) => {
    try { await apiClient.patch(`/api/projects/${projectId}/test-suites/${id}/${archived ? "restore" : "archive"}`); toast.success(archived ? "Restored" : "Archived"); load(); }
  ...
  const handleAddTc = async () => {
    ...
    try {
      await apiClient.post(`/api/projects/${projectId}/test-suites/${addTcModal}/test-cases`, { testCaseId: selectedTcId });
  ...
  const handleRemoveTc = async (suiteId: string, testCaseId: string) => {
    if (!projectId) return;
    try {
      await apiClient.delete(`/api/projects/${projectId}/test-suites/${suiteId}/test-cases`, { data: { testCaseId } });
  ...
  const handleExecute = async () => {
    ...
    try {
      await apiClient.post(`/api/projects/${projectId}/test-suites/${execModal}/execute`, { executionMode: execMode });
  ...
  const getSuiteTestCaseIds = (suiteId: string) => {
    ...
    return suite?.testCases?.map((tc: any) => tc.testCase?.id || tc.testCaseId) || [];
  };

  const priorityColor = (p: string) =>
    p === "CRITICAL" ? "red" : p === "HIGH" ? "orange" : p === "MEDIUM" ? "yellow" : "gray";
    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{suites.length} suite{suites.length !== 1 ? "s" : ""}</p>
        </div>
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  {canEdit && !suite.isArchived && (
                    <>
                      ...
                      <Button variant="secondary" size="sm" onClick={() => { setAddTcModal(suite.id); loadTestCases(); }}>+ Add</Button>
                ...
                <div style={{ borderTop: "1px solid var(--border-subtle)" }}>
                  {!suite.testCases?.length ? (
                    <div style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: "0.85rem" }}>
                      ...
                      {canEdit && (
                        <button
                          onClick={() => { setAddTcModal(suite.id); loadTestCases(); }}
                          style={{ marginLeft: 8, color: "var(--accent)", background: "none", border: "none", cursor: "pointer", fontSize: "0.85rem" }}
                        >
                          + Add one
                        </button>
                      )}
                        ...
                        {suite.testCases.map((tc: any, i: number) => {
                          ...
                          return (
                            <tr key={tc.id}>
                              ...
                              <td>{testCase.priority ? <Badge color={priorityColor(testCase.priority) as any}>{testCase.priority}</Badge> : "—"}</td>
      ...
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="Create Test Suite" size="sm">
        <FormField label="Name" required>
          <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. User Authentication Suite" />
        </FormField>
      ...
      <Modal open={!!addTcModal} onClose={() => { setAddTcModal(null); setSelectedTcId(""); }} title="Add Test Case to Suite" size="md">
        <FormField label={`Test Cases${selectedTcId ? " (1 selected)" : ""}`} required>
          <div style={{ maxHeight: 300, overflowY: "auto", border: "1px solid var(--border)", borderRadius: "var(--radius-md)", background: "var(--bg-base)" }}>
            {testCases.length === 0 ? (
              <div style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: "0.82rem" }}>
                <Spinner size={16} /> Loading...
              </div>
            ) : (() => {
              const alreadyAdded = addTcModal ? getSuiteTestCaseIds(addTcModal) : [];
              const available = testCases.filter((tc: any) => !alreadyAdded.includes(tc.id));
              return available.length === 0 ? (
                <div style={{ padding: 20, textAlign: "center", color: "var(--text-muted)", fontSize: "0.82rem" }}>All test cases already added to this suite</div>
                ...
                return (
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <div style={{ fontSize: "0.7rem", color: "var(--text-muted)", display: "flex", gap: 8, marginTop: 1 }}>
                        <span style={{ fontFamily: "var(--font-mono)" }}>{tc.testCaseId}</span>
                        <span>·</span>
                        <span>{tc.module}</span>
                        <span>·</span>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 16 }}>
          <Button variant="secondary" onClick={() => { setAddTcModal(null); setSelectedTcId(""); }}>Cancel</Button>
          <Button onClick={handleAddTc} disabled={!selectedTcId}>Add to Suite</Button>
        </div>
      </Modal>
