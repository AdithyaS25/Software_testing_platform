...
export default function ProjectMembersTab({ project, onRefresh }: Props) {
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message ?? 'Failed to add member — check the User ID');
    } finally {
      setAdding(false);
    }
  };
    ...
    <div style={{ maxWidth: 680 }}>
      ...
      {isOwner && (
        <div style={{
          ...
          <h3 style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4 }}>
            Add Member
          </h3>
          <p style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginBottom: 14 }}>
            Enter the User ID of the person to add. You can find User IDs in your database or admin panel.
          </p>
      ...
      <div style={{
        ...
        {members.length === 0 ? (
          ...
          members.map((member, i) => {
            const roleStyle = ROLE_COLORS[member.user?.role] ?? { bg: 'var(--bg-elevated)', color: 'var(--text-muted)' };
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    <div style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>
                      ...
                      {isCurrentUser && (
                        <span style={{ marginLeft: 6, fontSize: '0.7rem', color: 'var(--text-muted)', fontWeight: 400 }}>(you)</span>
                      )}
                    </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {isProjectOwner && (
                    <span style={{
                      fontSize: '0.7rem', fontWeight: 600, padding: '3px 8px',
                      borderRadius: 100, background: 'var(--warning-muted)', color: 'var(--warning)',
                    }}>
                      Owner
                    </span>
                  )}
