...
export default function ProjectMilestonesTab({ projectId }: Props) {
  ...
  const handleSave = async () => {
    if (!form.name || !form.targetDate) { toast.error('Name and target date are required'); return; }
    setSaving(true);
    try {
      const payload = {
        name: form.name, targetDate: form.targetDate, status: form.status,
  ...
  // Group by status
  const groups: Record<string, any[]> = { IN_PROGRESS: [], UPCOMING: [], COMPLETED: [], MISSED: [] };
  milestones.forEach(m => { if (groups[m.status]) groups[m.status].push(m); });
  ...
  return (
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24 }}>
        ...
        <Button onClick={openCreate} icon="+">Add Milestone</Button>
      </div>

      {milestones.length === 0 ? (
        <EmptyState
          icon="🎯" title="No milestones yet"
            ...
            return (
              <div key={status}>
                ...
                <div style={{
                  ...
                  <span style={{ fontSize: '0.9rem' }}>{STATUS_ICON[status]}</span>
                  <span style={{
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                  {group.map(m => {
                    ...
                    const barWidth = target ? Math.min(100, (passRate / target) * 100) : 0;
                    ...
                    return (
                      ...
                      >
                        ...
                        {/* Meta row */}
                        <div style={{ display: 'flex', gap: 14, fontSize: '0.75rem', flexWrap: 'wrap', marginBottom: target ? 14 : 0 }}>
                          {m.targetDate && (
                          ...
                          {m.testRuns?.length > 0 && (
                            <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
                              ▷ {m.testRuns.length} run{m.testRuns.length !== 1 ? 's' : ''} linked
                            </span>
                          )}
                          {target != null && (
                            <span style={{ color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        {target != null && (
                          <div>
                            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, fontSize: '0.72rem' }}>
                              ...
                              <span style={{
                                color: passRate >= target ? 'var(--success)' : 'var(--text-secondary)',
                                fontWeight: 600,
                              }}>
