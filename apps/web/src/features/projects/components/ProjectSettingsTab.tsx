...
export default function ProjectSettingsTab({ project, onUpdated }: Props) {
  ...
  const [fieldForm, setFieldForm] = useState<{
    name: string; fieldType: CustomFieldType; options: string; required: boolean;
  ...
  const handleAddField = async () => {
    ...
    try {
      ...
      setFieldForm({ name: '', fieldType: 'TEXT', options: '', required: false });
      onUpdated();
    } finally {
  ...
  const sectionTitle: React.CSSProperties = {
    fontSize: '0.875rem', fontWeight: 700, color: 'var(--text-primary)', marginBottom: 4,
  };
  const btn = (variant: 'primary' | 'danger' = 'primary'): React.CSSProperties => ({
    padding: '7px 16px', fontSize: '0.85rem', fontWeight: 500,
    borderRadius: 'var(--radius-md)', border: '1px solid',
    cursor: 'pointer', fontFamily: 'var(--font-sans)',
    ...(variant === 'primary' ? {
      background: 'var(--accent)', borderColor: 'var(--accent)', color: '#fff',
    } : {
      background: 'var(--danger-muted)', borderColor: 'rgba(255,77,106,0.3)', color: 'var(--danger)',
    }),
  });
    ...
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: 14 }}>
          ...
          <textarea value={info.description} onChange={e => setInfo(p => ({ ...p, description: e.target.value }))} rows={3} style={{ resize: 'vertical' }} />
        </div>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: 12 }}>
          {(project.environments ?? []).length === 0 ? (
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>No environments yet.</p>
          ) : (
            (project.environments ?? []).map((env: ProjectEnvironment) => (
              <div key={env.id} style={{
                ...
                <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>{env.name}</span>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{env.url || '—'}</span>
                  <button style={{ ...btn('danger'), padding: '3px 10px', fontSize: '0.75rem' }} onClick={() => handleDeleteEnv(env.id)}>Remove</button>
                </div>
              </div>
            ))
          )}
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <input value={envForm.name} onChange={e => setEnvForm(p => ({ ...p, name: e.target.value }))} placeholder="Name (e.g. Staging)" style={{ flex: 1 }} />
          <input value={envForm.url}  onChange={e => setEnvForm(p => ({ ...p, url: e.target.value }))}  placeholder="URL (optional)"      style={{ flex: 1 }} />
          <button style={btn()} onClick={handleAddEnv} disabled={savingEnv}>{savingEnv ? '...' : 'Add'}</button>
        </div>
      </div>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom: 12 }}>
          {(project.customFields ?? []).length === 0 ? (
            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>No custom fields yet.</p>
          ) : (
            (project.customFields ?? []).map((field: ProjectCustomField) => (
              <div key={field.id} style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ fontSize: '0.875rem', fontWeight: 600, color: 'var(--text-primary)' }}>{field.name}</span>
                  <span style={{ fontSize: '0.72rem', color: 'var(--text-muted)' }}>{field.fieldType}</span>
                  {field.required && <span style={{ fontSize: '0.72rem', color: 'var(--danger)' }}>Required</span>}
                </div>
                <button style={{ ...btn('danger'), padding: '3px 10px', fontSize: '0.75rem' }} onClick={() => handleDeleteField(field.id)}>Remove</button>
              </div>
            ))
          )}
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginBottom: 8 }}>
          <input value={fieldForm.name} onChange={e => setFieldForm(p => ({ ...p, name: e.target.value }))} placeholder="Field name" />
          <select value={fieldForm.fieldType} onChange={e => setFieldForm(p => ({ ...p, fieldType: e.target.value as CustomFieldType }))}>
      ...
      {project.status !== 'ARCHIVED' && (
        <div style={{ ...card, border: '1px solid rgba(255,77,106,0.3)' }}>
          ...
          <button style={btn('danger')} onClick={handleArchive}>Archive Project</button>
        </div>
      )}
    </div>
  );
