...
export default function MilestoneFormModal({
  ...
  useEffect(() => {
    ...
    apiClient
      ...
      .then(({ data }: { data: { data: TestRunOption[] } }) =>
        setTestRunOptions(data.data ?? [])
      )
      .catch(() => {});
  }, [projectId]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        passRateTarget: form.passRateTarget
          ? parseFloat(form.passRateTarget)
          : undefined,
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((p) => ({ ...p, passRateTarget: e.target.value }))
                }
                placeholder="e.g. 95"
                className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              />
