...
export const TestRunsPage = () => {
  ...
  const loadTestCases = async () => {
    ...
    try {
      const r = await apiClient.get(`/api/projects/${projectId}/test-cases?limit=100`);
      setTestCases(r.data.items || r.data.data || r.data || []);
    } catch { /* silently fail — user can still type IDs manually */ }
  };
  ...
  const handleCreate = async () => {
    ...
    if (form.selectedIds.length === 0) { toast.error("Select at least one test case"); return; }
    setSaving(true);
    try {
      ...
      setForm({ name: "", description: "", startDate: "", endDate: "", selectedIds: [] });
      load();
    } catch (e: any) {
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      await apiClient.delete(`/api/projects/${projectId}/test-runs/${deleteTarget.id}`);
    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">{runs.length} test run{runs.length !== 1 ? "s" : ""}</p>
        </div>
        {canCreate && (
          <Button icon="+" onClick={() => { setCreateModal(true); loadTestCases(); }}>
            New Test Run
          </Button>
        )}
      </div>
      ...
      {loading ? (
        ...
        <EmptyState
          ...
          action={canCreate ? <Button onClick={() => { setCreateModal(true); loadTestCases(); }}>Create Test Run</Button> : undefined}
        />
      ) : (
      ...
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="Create Test Run" size="md">
        ...
        <FormField label={`Test Cases ${form.selectedIds.length > 0 ? `(${form.selectedIds.length} selected)` : ""}`} required>
          <div style={{
              ...
              testCases.map((tc: any) => {
                ...
                return (
                  <div
                    ...
                    onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.background = selected ? "rgba(16,185,129,0.06)" : "transparent"; }}
                  >
                    {/* Checkbox */}
                    <div style={{
          ...
          {testCases.length > 0 && (
            <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
              <button
                onClick={() => setForm(p => ({ ...p, selectedIds: testCases.map((tc: any) => tc.id) }))}
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete Test Run" size="sm">
        <p style={{ color: "var(--text-secondary)", marginBottom: 6, fontSize: "0.9rem" }}>
