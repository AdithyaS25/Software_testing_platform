  ...
  return (
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      {loading ? (
        ...
        <EmptyState icon="▷" title="No test runs" desc="Create a test run to track execution cycles" action={canCreate ? <Button onClick={() => setCreateModal(true)}>Create Test Run</Button> : undefined} />
      ) : (
                  ...
                  <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
                    <div style={{ flex: 1 }}>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
                        <button onClick={() => setExpanded(expanded === run.id ? null : run.id)} style={{ background: "none", border: "none", cursor: "pointer", color: "var(--text-muted)", fontSize: "0.75rem", padding: 0 }}>
                          {expanded === run.id ? "▼" : "▶"}
                        </button>
                        <span style={{ fontWeight: 600, color: "var(--text-primary)" }}>{run.name}</span>
                        <Badge color="blue">{total} tests</Badge>
                        {run.status && <Badge color={run.status === "COMPLETED" ? "green" : run.status === "IN_PROGRESS" ? "yellow" : "gray"}>{run.status?.replace(/_/g, " ")}</Badge>}
                      </div>
                      {run.description && <p style={{ fontSize: "0.8rem", color: "var(--text-muted)", marginLeft: 18, marginBottom: 8 }}>{run.description}</p>}
                    ...
                    <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                      <Button variant="secondary" size="sm" onClick={() => nav(`/reports?testRunId=${run.id}`)}>📊 Report</Button>
                    </div>
                  </div>
                </div>
                ...
                {expanded === run.id && run.testCases?.length > 0 && (
                  <div style={{ borderTop: "1px solid var(--border-subtle)" }}>
                    <table className="tt-table">
                      <thead><tr><th>Test Case</th><th>Status</th><th>Assigned To</th></tr></thead>
                      <tbody>
                        {run.testCases.map((tc: any) => (
                          <tr key={tc.id}>
                            <td style={{ fontWeight: 500 }}>{tc.testCase?.title || tc.title || tc.id}</td>
                            <td><Badge color={tc.status === "PASSED" ? "green" : tc.status === "FAILED" ? "red" : tc.status === "BLOCKED" ? "yellow" : "gray"}>{tc.status || "PENDING"}</Badge></td>
                            <td><span style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>{tc.assignedTo?.email?.split("@")[0] || "—"}</span></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <Modal open={createModal} onClose={() => setCreateModal(false)} title="Create Test Run" size="md">
        <FormField label="Name" required><input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))} placeholder="e.g. Sprint 5 Regression" required /></FormField>
        <FormField label="Description"><textarea value={form.description} onChange={e => setForm(p => ({ ...p, description: e.target.value }))} rows={2} style={{ resize: "vertical" }} /></FormField>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          <FormField label="Start Date" required><input type="date" value={form.startDate} onChange={e => setForm(p => ({ ...p, startDate: e.target.value }))} /></FormField>
          <FormField label="End Date" required><input type="date" value={form.endDate} onChange={e => setForm(p => ({ ...p, endDate: e.target.value }))} /></FormField>
        </div>
        <FormField label="Test Case IDs">
          <textarea value={form.testCaseIds} onChange={e => setForm(p => ({ ...p, testCaseIds: e.target.value }))} placeholder="Paste comma-separated test case IDs..." rows={3} style={{ resize: "vertical" }} />
        </FormField>
