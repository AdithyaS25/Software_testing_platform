      ...
      <form onSubmit={handleSubmit}>
        <div style={{
          ...
          <FormField label="Description" required>
            <textarea value={form.description} onChange={setField("description")} rows={3} required />
          </FormField>

          <FormField label="Steps to Reproduce">
            <textarea value={form.stepsToReproduce} onChange={setField("stepsToReproduce")} rows={3} />
          </FormField>

          <FormField label="Expected Behavior">
            <textarea value={form.expectedBehavior} onChange={setField("expectedBehavior")} rows={2} />
          </FormField>

          <FormField label="Actual Behavior">
            <textarea value={form.actualBehavior} onChange={setField("actualBehavior")} rows={2} />
          </FormField>
          ...
          <FormField label="Environment">
            <input value={form.environment} onChange={setField("environment")} />
          </FormField>

          <FormField label="Affected Version">
            <input value={form.affectedVersion} onChange={setField("affectedVersion")} />
          </FormField>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
          <Button type="button" variant="secondary" onClick={() => nav(`/projects/${projectId}/bugs`)}>
            Cancel
          </Button>
          <Button loading={loading} type="submit">Create Bug</Button>
        </div>
      </form>
    </div>
  );
