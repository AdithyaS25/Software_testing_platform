    ...
    <div style={{ animation: "fadeIn 0.3s ease" }}>
      ...
      <div className="filter-bar">
        ...
        <Select value={statusFilter} onChange={setStatusFilter} placeholder="All Statuses" options={[
          ...
          { value: "FIXED", label: "Fixed" }, { value: "VERIFIED", label: "Verified" }, { value: "CLOSED", label: "Closed" },
        ...
        <Select value={priorityFilter} onChange={setPriorityFilter} placeholder="All Priorities" options={[
          ...
          { value: "P3-MEDIUM", label: "P3 Medium" }, { value: "P4-LOW", label: "P4 Low" },
        ]} />
      ...
      {loading ? (
        ...
        <EmptyState icon="⚠" title="No bugs found" desc="No bug reports match your current filters" />
      ) : (
          ...
          <table className="tt-table">
            <thead>
              <tr>
                ...
                <th>Status</th><th>Assigned To</th><th>Age</th><th style={{ textAlign: "right" }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(bug => (
                <tr key={bug.id}>
                  <td><span style={{ fontFamily: "var(--font-mono)", fontSize: "0.75rem", color: "var(--text-muted)" }}>{bug.bugId || bug.id?.slice(0, 8)}</span></td>
                  <td>
                    <button onClick={() => nav(`/bugs/${bug.id}`)} style={{ background: "none", border: "none", color: "var(--text-primary)", cursor: "pointer", fontWeight: 500, fontSize: "0.875rem", textAlign: "left", fontFamily: "var(--font-sans)", padding: 0 }}
                  ...
                  <td><PriorityBadge value={bug.priority?.replace("P1-URGENT", "CRITICAL").replace("P2-HIGH", "HIGH").replace("P3-MEDIUM", "MEDIUM").replace("P4-LOW", "LOW")} /></td>
                  <td><SeverityBadge value={bug.severity} /></td>
                  <td><StatusBadge value={bug.status} /></td>
                  <td><span style={{ color: "var(--text-secondary)", fontSize: "0.8rem" }}>{bug.assignedTo?.email?.split("@")[0] || <span style={{ color: "var(--text-muted)" }}>Unassigned</span>}</span></td>
                  <td><span style={{ color: "var(--text-muted)", fontSize: "0.78rem" }}>{ageLabel(bug.createdAt)}</span></td>
                  <td>
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      <Button variant="ghost" size="sm" onClick={() => nav(`/bugs/${bug.id}`)}>View</Button>
                    </div>
                  </td>
                </tr>
              ))}
