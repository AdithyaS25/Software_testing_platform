    ...
    <div style={{ animation: "fadeIn 0.4s ease" }}>
      ...
      <div style={{ marginBottom: 32, position: "relative" }}>
        <div style={{
          ...
          background: "radial-gradient(circle, rgba(120,87,255,0.08) 0%, transparent 70%)",
      ...
      <div style={{
        ...
        {isTester && <>
          <StatCard label="Test Runs"  value={totalRuns} icon="▷" color="blue"   sub="Total cycles" />
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
        ...
        {isTester && passRate != null && (
          <PassRateBar passRate={passRate} totalExec={typeof totalExec === "number" ? totalExec : 0} />
        )}
      </div>
...
const BugStatusBreakdown = ({ summary }: { summary: any }) => {
  const statuses = [
    ...
    { label: "In Progress", value: summary.inProgressBugs ?? summary.openBugs ?? 0, color: "var(--info)" },
    { label: "Critical",    value: summary.criticalBugs  ?? 0, color: "var(--danger)" },
...
// ── Pass Rate Bar ─────────────────────────────────────────────────────────────
const PassRateBar = ({ passRate, totalExec }: { passRate: number; totalExec: number }) => {
  ...
  return (
    <div className="glass-card" style={{ padding: 20 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
        ...
        <span style={{
          fontSize: "0.68rem", fontWeight: 700, color: hex,
          background: `${hex}18`, padding: "2px 8px",
          borderRadius: "100px", border: `1px solid ${hex}40`,
        }}>{label}</span>
      </div>
      ...
      <div style={{ display: "flex", alignItems: "flex-end", gap: 6, marginBottom: 16 }}>
        <span style={{
          ...
          color: hex, fontFamily: "var(--font-display)", letterSpacing: "-0.04em",
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        {["0", "25%", "50%", "75%", "100%"].map(t => (
          <span key={t} style={{ fontSize: "0.58rem", color: "var(--text-muted)" }}>{t}</span>
        ))}
      </div>
    </div>
  );
