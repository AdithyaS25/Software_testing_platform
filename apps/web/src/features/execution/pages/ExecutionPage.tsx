...
export const ExecutionPage = () => {
  ...
  const [selectedRunId, setSelectedRunId] = useState(searchParams.get("testRunId") || "");
  const [selectedTcId,  setSelectedTcId]  = useState(testCaseId || "");
  const [testCases,     setTestCases]     = useState<any[]>([]);

  const [bugForm, setBugForm] = useState({
    title: "", description: "", severity: "MAJOR", priority: "P3_MEDIUM",
  });

  // Load test runs and test cases for the picker modal
  useEffect(() => {
    if (!projectId) return;
    apiClient.get(`/api/projects/${projectId}/test-runs`)
      .then(r => setTestRuns(r.data.data || r.data || []));
    apiClient.get(`/api/projects/${projectId}/test-cases`)
      .then(r => setTestCases(r.data.data?.items || r.data.items || r.data.data || r.data || []));
  }, [projectId]);
  ...
  const startExecution = async (tcId: string, runId: string) => {
    ...
    try {
      ...
      const tcRes  = await apiClient.get(`/api/projects/${projectId}/test-cases/${tcId}`);
      const tcData = tcRes.data.data || tcRes.data;
      setTc(tcData);

      // Create execution — backend requires testCaseId + testRunId
      const exRes  = await apiClient.post(`/api/projects/${projectId}/executions`, {
        testCaseId: tcId,
        testRunId:  runId,
      });
  ...
  const saveStep = async (stepId: string) => {
    ...
    await apiClient.patch(`/api/projects/${projectId}/executions/${execution.id}`, {
  ...
  const handleComplete = async () => {
    ...
    try {
      ...
      await apiClient.patch(`/api/projects/${projectId}/executions/${execution.id}`, { steps });
      const res    = await apiClient.post(`/api/projects/${projectId}/executions/${execution.id}/complete`);
  ...
  const statusColors: Record<StepStatus, string> = {
    PENDING: "var(--text-muted)", PASS: "var(--success)",
    FAIL: "var(--danger)", BLOCKED: "var(--warning)", SKIPPED: "var(--text-muted)",
  };

  // ── Test Run / Test Case Picker Modal ──────────────────────────────────
  if (runModal) {
    return (
      <div style={{ animation: "fadeIn 0.3s ease", maxWidth: 520, margin: "60px auto" }}>
        <div style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "var(--radius-lg)", padding: 32 }}>
          <h2 style={{ fontSize: "1.1rem", fontWeight: 700, color: "var(--text-primary)", marginBottom: 6 }}>Start Execution</h2>
          <p style={{ fontSize: "0.85rem", color: "var(--text-muted)", marginBottom: 24 }}>Select a test case and test run to begin execution.</p>

          <FormField label="Test Case" required>
            <select value={selectedTcId} onChange={e => setSelectedTcId(e.target.value)}
              style={{ width: "100%" }}>
          ...
          <div style={{ display: "flex", gap: 10, justifyContent: "flex-end", marginTop: 20 }}>
            ...
            <Button onClick={() => startExecution(selectedTcId, selectedRunId)}>Start Execution</Button>
          </div>
        </div>
      </div>
    );
    ...
    <div style={{ animation: "fadeIn 0.3s ease", maxWidth: 900 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          ...
          <Button loading={completing} onClick={handleComplete}>Complete Execution</Button>
        </div>
      </div>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 14 }}>
                    ...
                    {step.testData && (
                      <div>
                        ...
                        <p style={{ fontSize: "0.875rem", color: "var(--info, #60a5fa)", fontFamily: "var(--font-mono)" }}>{step.testData}</p>
                      </div>
                    )}
                  </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
          ...
          <FormField label="Priority">
            <select value={bugForm.priority} onChange={e => setBugForm(p => ({ ...p, priority: e.target.value }))}>
              ...
              {[["P1_URGENT","P1 Urgent"],["P2_HIGH","P2 High"],["P3_MEDIUM","P3 Medium"],["P4_LOW","P4 Low"]].map(([v,l]) =>
