...
export const createTestSuiteController: RequestHandler = async (
  ...
  if (parentId) {
    ...
    if (!parentSuite)
      return res
        .status(400)
        .json({ message: 'Parent suite not found in this project' });
  }
...
export const executeSuiteController: RequestHandler = async (req, res) => {
  ...
  const projectId = Array.isArray(req.params.projectId)
    ? req.params.projectId[0]
    : req.params.projectId;
  const suiteId = Array.isArray(req.params.suiteId)
    ? req.params.suiteId[0]
    : req.params.suiteId;
  ...
  return res
    .status(201)
    .json({ message: 'Suite execution started', data: result });
};
...
export const completeSuiteExecutionController: RequestHandler = async (
  ...
  const suiteExecutionId = Array.isArray(req.params.suiteExecutionId)
    ? req.params.suiteExecutionId[0]
    : req.params.suiteExecutionId;
  if (!suiteExecutionId)
    return res.status(400).json({ message: 'Invalid execution ID' });
  const result = await completeSuiteExecution(suiteExecutionId);
  return res
    .status(200)
    .json({ message: 'Suite execution completed', data: result });
};

/* ── Get Suite Execution Report ──────────────────────────── */
export const getSuiteExecutionReportController: RequestHandler = async (
  req,
  res
) => {
  const suiteExecutionId = Array.isArray(req.params.suiteExecutionId)
...
export const reorderSuiteTestCasesController: RequestHandler = async (
  ...
  const projectId = Array.isArray(req.params.projectId)
    ? req.params.projectId[0]
    : req.params.projectId;
  const suiteId = Array.isArray(req.params.id)
    ? req.params.id[0]
    : req.params.id;
  ...
  if (!Array.isArray(orderedTestCaseIds) || orderedTestCaseIds.length === 0) {
    return res
      .status(400)
      .json({ message: 'orderedTestCaseIds must be a non-empty array' });
  }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (let i = 0; i < orderedTestCaseIds.length; i++) {
        await tx.testSuiteTestCase.update({
          where: {
            suiteId_testCaseId: { suiteId, testCaseId: orderedTestCaseIds[i] },
          },
          data: { position: i + 1 },
        });
      }
...
export const cloneSuiteController: RequestHandler = async (req, res) => {
  ...
  return res
    .status(201)
    .json({ message: 'Suite cloned successfully', data: cloned });
};
...
export const addTestCaseToSuiteController: RequestHandler = async (
  ...
  if (!projectId || !suiteId || !testCaseId) {
    return res
      .status(400)
      .json({ message: 'Project ID, Suite ID and Test Case ID required' });
  }
  ...
  const testCase = await prisma.testCase.findFirst({
    where: {
      projectId,
      OR: [{ id: testCaseId }, { testCaseId: testCaseId }],
    },
    select: { id: true },
  });
  ...
  if (!testCase) {
    return res
      .status(404)
      .json({ message: `Test case "${testCaseId}" not found in this project` });
  }
  ...
  if (existing)
    return res.status(409).json({ message: 'Test case already in this suite' });
