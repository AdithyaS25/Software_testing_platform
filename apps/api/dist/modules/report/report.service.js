...
exports.generateTestExecutionReport = generateTestExecutionReport;
const prisma_1 = require("../../prisma");
async function generateTestExecutionReport(projectId, testRunId) {
    const testRun = await prisma_1.prisma.testRun.findFirst({
        where: {
            id: testRunId,
            projectId
        },
    ...
    // SUMMARY
    // =============================
    const totalExecuted = await prisma_1.prisma.execution.count({
        where: {
            testRunId,
            overallResult: { not: null }
        }
    });
    const statusBreakdown = await prisma_1.prisma.execution.groupBy({
        by: ["overallResult"],
        where: {
            testRunId,
            overallResult: { not: null }
    ...
    statusBreakdown.forEach((item) => {
        if (item.overallResult === "PASS")
            passed = item._count.overallResult;
        if (item.overallResult === "FAIL")
            failed = item._count.overallResult;
        if (item.overallResult === "BLOCKED")
            blocked = item._count.overallResult;
        if (item.overallResult === "SKIPPED")
            skipped = item._count.overallResult;
    });
    ...
    // EXECUTION BY TESTER
    // =============================
    const executionByTesterRaw = await prisma_1.prisma.execution.groupBy({
        by: ["executedById"],
        where: { testRunId },
        _count: { id: true }
    });
    const executionByTester = await Promise.all(executionByTesterRaw.map(async (entry) => {
    ...
    // =============================
    const timelineRaw = await prisma_1.prisma.execution.findMany({
        where: {
            testRunId,
    ...
    // =============================
    const failedExecutions = await prisma_1.prisma.execution.findMany({
        where: {
            testRunId,
            overallResult: "FAIL"
        },
